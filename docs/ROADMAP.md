...
| **P5** | Menu MVP: preview character + camera, equipment + item browser panels, OK/Cancel apply | **See `docs/ARSENAL_STATUS_2026-07-14.md` for current detail.** Tabs, item grid, double-click-to-add all confirmed working live. Camera architecture rewritten (attached camera entity + world camera slot, no more per-frame manual positioning). Open: preview character's life-state/stance not confirmed changing (`ForceStanceUp`/`ActivateAI` both read as no-ops in the latest log), vest/backpack fail to place during loadout mirror (likely downstream of the same issue), one native engine crash observed post-close (cause unconfirmed, defensive mitigation applied). |
